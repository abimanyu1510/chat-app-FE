...
import v1 from "../../public/talking3.png"
...
function Navbar() {
  ...
  return (
    <NavContainer>
      <Link to="/">
        <NavLogo>
          <NavImage src={v1} alt="../../public/talking3.png"  id='logo'/>
          <NavBrand id='Chatty'>Chatty</NavBrand>
          {show && onlineUsers && <NavCount> Users：{onlineUsers.length || 0}</NavCount>}
        </NavLogo>
      </Link>
      {user ? (
        <NavUser>
          Welcome! <span style={{"color":"#40E0D0 ","textShadow":"1px 1px 1px #CACFD2 "}} id="name">{user.name}</span>
        </NavUser>
      ) : null}
      <NavIcons>
        <NavIcon id='light'>
